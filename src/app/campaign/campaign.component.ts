...
export class CampaignComponent implements OnInit {
  ...
  campagneCollectionRef: AngularFirestoreCollection<Campagne>;
  campagne$: Observable<Campagne[]>;
  Item: any;
  ...
  constructor(private auth: AuthService, private router: Router, private afs: AngularFirestore) {
    this.auth.user.forEach((data) => {
     ...
     this.campagneCollectionRef = this.afs.collection<Campagne>(this.uid);
    this.campagne$ = this.campagneCollectionRef.snapshotChanges().pipe(
      map(
        actions => {
          return actions.map(action => {
            const data = action.payload.doc.data() as Campagne;
...
this.canvas = new fabric.Canvas('canvas', {
  ...
  selectionBorderColor: 'blue'
    ...
    // let canvasElement: Item = document.getElementById('canvas');
